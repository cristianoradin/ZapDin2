"""
Launcher ZapDin Monitor
Inicia o servidor uvicorn em background e abre o navegador em modo app.
Compilado com PyInstaller --onefile --noconsole para gerar ZapDin-Monitor.exe
"""
import os
import subprocess
import sys
...
def _find_python(root: str) -> str:
    candidates = [
        os.path.join(root, "monitor", ".venv", "Scripts", "python.exe"),  # Windows — monitor/.venv
        os.path.join(root, "monitor", ".venv", "bin", "python"),           # Unix — monitor/.venv
...
def main() -> None:
    ...
    log_path = os.path.join(root, "logs", "monitor.log")
